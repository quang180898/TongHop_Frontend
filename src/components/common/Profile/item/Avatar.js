...
const Avatar = ({ data, setDataImage }) => {
    ...
    useEffect(() => {
        if (updateAccount) {
            if (updateAccount.success) {
                setState({ ...state, imgOld: updateAccount.detail.customer_image_base64 })
                showNotification.success({ message: 'Cập nhật thành công', title: 'success' })
                setTimeout(() => {
                    setVisibleModal(false)
                }, 1000);
            } else {
                showNotification.error({ message: updateAccount.detail, title: 'error' })
            }
        }
    }, [updateAccount])
    ...
    const onSaveImageDone = () => {
        dispatch(accountAction.postUpdateAccount({user_id: parseInt(accountId),image: imgTest, image_name: imgTest.name }))
        setState({
            ...state,
            openCrop: false,
            imgOld: null
